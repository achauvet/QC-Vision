...
int main (int argc, char *argv[])
{
	...
	if ((pnl_handle = LoadPanel (0, "ERG.uir", ERG_panel)) < 0)
		return -1;
								...
								void *callbackData, int eventData1, int eventData2)
{
	...
	switch (event)
	{
		case EVENT_COMMIT:
			...
			GetCtrlVal(ERG_panel, ERG_panel_npulses, &Npulses);
							...
							void *callbackData, int eventData1, int eventData2)
{
	...
	switch (event)
	{
		case EVENT_COMMIT:
			...
			GetCtrlVal(ERG_panel, ERG_panel_nsec, &Nsec);
